...
<template>
  <section class="chuck" id="chuck">
    <h1><i class="icn icn-bullhorn"></i> Chuck Jokes</h1>
...
export default {
  ...
  computed: {
    bot() {
      return this.$store.getters['chuck/bot'];
    },
    communicating() {
      return this.$store.getters['chuck/communicating'];
    },
    joke() {
      return this.$store.getters['chuck/joke'];
    }
  },
  methods: {
    askChuck() {
      this.$store.dispatch('chuck/askChuck');
    }
  },
  created() {},
  destroyed() {},
...
// custom template styles
  ...
  .chuck
    ...
    .chuck-joke
      ...
      border-bottom: none
      border-radius: 5rem
      background-color: lighten($colors.charcoal, 20%)
